...
**You are in `createos-cli` — Go CLI (`createos`).** `createos sandbox` hits the same control plane as the SDKs (`https://api.sb.createos.sh`). Every plugin under `createos-plugin` shells out to this binary, so a renamed command or flag breaks them silently — check them before shipping one.
...
| **createos-cli** ← you are here | this repo | Go CLI (`createos`) | 🌐 public | commands, flags, help/UX text |
